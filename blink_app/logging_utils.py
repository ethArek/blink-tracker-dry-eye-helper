...
def setup_logging(output_dir: str) -> tuple[logging.Logger, logging.Logger, logging.Logger]:
    ...
    # Ensure there is exactly one console StreamHandler attached to blink_logger
    blink_logger.handlers = [
        h for h in blink_logger.handlers
        if not (isinstance(h, logging.StreamHandler) and not isinstance(h, logging.FileHandler))
    ]
    blink_logger.addHandler(console_handler)
